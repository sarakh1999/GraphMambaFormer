...
def check_data_integrity(chk: Checker, dataset) -> None:
    ...
    cigar_ok = True
    for r in progress(main, desc="check CIGAR", unit="read", leave=False):
    ...
    m = s = i = d = 0
    for r in progress(main, desc="error-rate stats", unit="read", leave=False):
        for op, n in r.cigar:
            if op == "=":
                m += n
            elif op == "X":
    ...
    seed_ok = True
    for r in progress(main, desc="check seeds", unit="read", leave=False):
