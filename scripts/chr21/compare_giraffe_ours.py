...
def _bam_stats(path: str) -> dict | None:
    ...
    with pysam.AlignmentFile(path, "rb") as bam:
        for a in progress(bam.fetch(until_eof=True),
                          desc=f"stats {os.path.basename(path)}", unit="aln", leave=False):
...
def _vcf_counts(path: str) -> dict | None:
    ...
    with pysam.VariantFile(path) as vcf:
        for rec in progress(vcf, desc=f"count {os.path.basename(path)}", unit="var", leave=False):
